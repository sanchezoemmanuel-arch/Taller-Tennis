...
El proyecto está desarrollado y probado sobre **Expo SDK 57 (React Native 0.86)**.
...
# 2. Alinear las versiones nativas con tu SDK
npx expo install --fix

# 3. Verificar que no haya problemas de configuración
npx expo-doctor
...
Escanea el código QR con Expo Go. El teléfono y el computador deben estar en la misma red WiFi. La bandera `-c` limpia la caché de Metro y solo es necesaria la primera vez o cuando los estilos de NativeWind no se apliquen.

> **Nota sobre Babel:** `babel.config.js` declara únicamente los presets de Expo y NativeWind. `babel-preset-expo` ya incluye automáticamente el plugin de Reanimated / Worklets, así que **no debe agregarse a mano**: declararlo dos veces rompe la compilación.

### Si Expo Go muestra "Project is incompatible with this version of Expo Go"

Significa que la versión de Expo Go del teléfono no coincide con la del proyecto. La forma más simple de resolverlo es subir el proyecto al SDK que tenga instalado Expo Go:

```bash
npx expo install expo@^57.0.0   # usa el número que indique la pantalla del teléfono
npx expo install --fix
npm install nativewind@latest
npx expo start -c
```

Para evitar que vuelva a ocurrir, desactiva la actualización automática de Expo Go en la tienda de aplicaciones.

### Notas de entorno en Windows

- No trabajes dentro de carpetas sincronizadas por OneDrive o Drive: la sincronización interfiere con `node_modules` y produce errores del tipo `The required package X cannot be found`. Usa una ruta local como `C:\dev\breakpoint`.
- Si aparece `Cannot find module 'X'` pese a estar instalado, suele ser porque npm lo dejó anidado dentro de otro paquete. Se corrige declarándolo explícitamente con `npx expo install X`.
...
npx expo install @react-navigation/native @react-navigation/native-stack \
  ...
  expo-secure-store expo-sqlite expo-asset @react-native-community/netinfo \
  @react-native-async-storage/async-storage babel-preset-expo
...
| `react-native-reanimated` + `react-native-gesture-handler` | Requeridos por el Drawer de React Navigation |
